...
def produce_failure(idlist):
    ...
    snfake_ids = list()
    cursor = None #db_connect.db_connect(ins.des_services, ins.des_db_section)
    try:
...
def classify(X, clf, imputer, scaler):
    ...
    X_feats_unscaled = X[features.FEATS]
    X_snobs = X['snobjid']
...
if __name__ == '__main__':
    ...
    formats = ['<i8'] + ['<f8'] * len(features.FEATS)
    X_dtype = np.dtype(list(zip(names, formats)))
